...
\begin{circuitikz}[american, scale=1.2]
    ...
    % Clock
    \node (clk) at (0, -2) {CLK};
    \draw (clk) -- ++(1,0) node[circ] (clk_tap) {} |- (FF0.pin 2);
    \draw (clk_tap) -| (FF1.pin 2);
    \draw (FF0.pin 3) -- ++(-0.5,0) coordinate (FF0_K) -- ++(0,1.5) node[vcc] (vcc) {$'1'$};
    \draw (FF0.pin 1) to[short,-*] (FF0.pin 1 -| FF0_K);
   ...
   % \draw (vcc) -| (FF0.pin 3); % K0
    ...
    % Wire x to XNOR
    \draw (xnor.in 1) -- ++(-2,0) node[left] (x) {$x$};
    ...
    \draw (FF1.pin 1) to[short,-*] (FF1.pin 1 -| logic_out);
