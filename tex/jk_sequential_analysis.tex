...
\begin{circuitikz}[american, scale=1.0]
    ...
    \draw (x_bus_top) -- ++(0, -1) coordinate (x_drop);
    \node[not port, rotate=-90, anchor=in] (INV_x) at (x_drop) {};
    \draw (INV_x.out) -- ++(0, -0.5) coordinate (x_bar_bus);
    ...
    \draw (A_out) -- ++(0, -6) coordinate (A_bot) -| (XOR_KB.in 1);
    ...
    % B to JA (JK_A.pin 1)
    % Loop UP and left
    \draw (B_out) -- ++(0, 6) coordinate (B_top) -| (JK_A.pin 1);
