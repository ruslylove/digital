...
\begin{circuitikz}[
    ...
    \node[register] at (3,0) (A) {$A$};

    \draw[<-] (A.lpin 1) -- ++(-2,0) node[left,label_text] (ALoad) {$ALoad$};
    \draw[<-] (A.lpin 2) -- ++(-2,0) node[left,label_text] (AClock) {$Clock$};
    \draw[->] (A.bpin 1) |- ++ (0.5,-1) coordinate (AdderInput);
    \node[adder, box only, anchor=west] (Adder) at (AdderInput) {};
    \draw[<-] (Adder.east) -| ++ (0.5,1) node[above,label_text] (three) {$'3'$};
    \draw[->] (Adder.south) -- ++ (0,-0.5) -| ++(-3,4) -| (A.tpin 1);
