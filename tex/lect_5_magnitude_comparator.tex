...
\begin{circuitikz}
    ...
    % Output Node for A > B
    \draw (gtOr.out) -- ++(0.5,0) coordinate (gtOut);
    ...
    % Output Node for A = B
    \draw (eqAnd.out) -- ++(0.5,0) coordinate (eqOut);
    ...
    \draw (gtOut) |- (ltNor.in 1);
    
    % Route eqAnd output (A = B) to NOR input 2
    \draw (eqOut) |- (ltNor.in 2);
    
    \draw (ltNor.out) -- ++(0.5,0) node[right] (altOut) {$A < B$};
    \draw (gtOut) to[short,*-] (gtOut -| altOut.west) node[right] {$A > B$};
    \draw (eqOut) to[short,*-] (eqOut -| altOut.west) node[right] {$A = B$};
