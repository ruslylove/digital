...
\begin{circuitikz}[american, scale=1.0]
    ...
    % Common Signals
    \node (clk_in) at (-4, -3) {CLK};
    \node (clr_in) at (-4, -3.5) {Clear};
    \node (mode_s1) at (-4, -4) {$S_1$};
    \node (mode_s0) at (-4, -4.5) {$S_0$};
    
    % Serial Inputs
    %\node (sir_in) at (-4, 0) {SIR (Shift Right)};
    %\node (sil_in) at (18, 0) {SIL (Shift Left)}; 
    ...
    \foreach \i in {3,2,1,0} {
        ...
        % Flip-Flop
        \node[flipflop D,flipflop def={nd=1}] (FF\i) at (\xpos, 0) {};
        ...
        \coordinate (mux_pos_\i) at ($(FF\i.pin 1) + (-0.5, 0)$);
       ...
       % \node[right, font=\tiny] at (MUX\i.lpin 1) {3}; % Topmost (usually input 0 or 3 depending on standard? TikZ defines lpin 1 as top)
        ...
        \node[right, font=\tiny][xshift=0.2cm] at (MUX\i.lpin 1) {3};
        \node[right, font=\tiny][xshift=0.2cm] at (MUX\i.lpin 2) {2};
        \node[right, font=\tiny][xshift=0.2cm] at (MUX\i.lpin 3) {1};
        \node[right, font=\tiny][xshift=0.2cm] at (MUX\i.lpin 4) {0};
        ...
        \node[circ] at (Q_tap) {};
        \draw (Q_tap) -- ++(0,2.5) node[above] {$A_\i$};
       ...
       % \draw (FF\i.s) -- ++(0,-0.2) coordinate (clr_pt);
        \draw (FF\i.down) -- (FF\i.down |- clr_in) node[circ] {};
    ...
    \draw (MUX3.lpin 3) -- ++(-0.5,0) node[left] {SIR (Shift Right)};
    ...
    \draw (clr_in) -| (FF0.down);
    \draw (mode_s1) -| (MUX0.bpin 1);
    \draw (mode_s0) -| (MUX0.bpin 2);
