...
\begin{circuitikz}[
    ...
    \node[left,label_text] at ($(A.blpin 1) + (-2,0)$) (ALoad) {$ALoad$};
    \draw[control_signal] (ALoad) -- (A.blpin 1);
    \node[left,label_text] at ($(A.blpin 2) + (-2,0)$) (AClock) {$Clock$};
    \draw[control_signal] (AClock) -- (A.blpin 2);
    \draw[bus] (A.bbpin 1) -- ++(0,-0.5) -| ++ (0.5,-1) coordinate (AdderInput);
    \node[adder, box only, anchor=btpin 1] (Adder) at (AdderInput) {+};
    \node[above,label_text] at ($(Adder.tpin 2) + (0.0,0.7)$) (three) {$'3'$}; 
