...
\begin{circuitikz}[american, scale=1.0]
    ...
    \node[above] at (TFF_A.n) {Flip-Flop A};
    \node[flipflop T] (TFF_B) at (10, -3) {};
    \node[above] at (TFF_B.n) {Flip-Flop B};
    ...
    % B to AND_TA (input 1)
    % Loop UP 
    \draw (B_out) -- ++(0, 5) coordinate (B_top) -| (AND_TA.in 1);
    
    % Output y = AB
    % AND gate connected to A and B
    % Place it to the right of FFs
    \path (TFF_A.pin 6) -- ++(3, -2.5) coordinate (y_pos);
    \node[and port] (AND_y) at (y_pos) {$AB$};
