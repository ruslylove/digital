...
\begin{tikzpicture}[
    ...
    % Inputs: MOSI, SCLK
    % Hold on SCLK=0 (X0)
    % Transition on SCLK=1 (X1)
    
    % States
    \node[state, initial] (s0) {$S_0$\\ (Reset)};
    \node[state, right of=s0] (s1) {$S_1$\\ (1)};f
    ...
    % S0: Start
    \path (s0) edge[loop above] node {X0, 01} (s0) % Hold or MOSI=0,SCLK=1
               edge node {11} (s1); % MOSI=1, SCLK=1
    ...
    \node[draw, rectangle, fill=white, align=left, font=\scriptsize, anchor=north] at (0,-5) (legend) {
        \textbf{Legend:} \\
        Transition Label: \textbf{MOSI, SCLK} \\
        \textbf{X0}: Any MOSI, SCLK=0 (Hold State) \\
